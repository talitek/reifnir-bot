...
public class UserRoleModule : BaseCommandModule
{
    ...
    {
        IEnumerable<UserRole> userRoles = await _userRoleService.GetRoleList();

        var sb = new StringBuilder();

        sb.AppendLine("**List of roles**");

        IOrderedEnumerable<IGrouping<UserRoleGroup?, UserRole>> roleGroups = userRoles
            .GroupBy(r => r.Group)
        ...
        foreach (IGrouping<UserRoleGroup?, UserRole> roleGroup in roleGroups)
        {
            ...
            foreach (UserRole userRole in roleGroup.ToList())
            {
                sb.Append($"* {userRole.Name}");

                if (userRole.UserRoleAliases.Any())
                {
                    string formattedAliasList = string.Join(", ", userRole.UserRoleAliases.Select(x => x.Alias));
    ...
    {
        AppDiscordRole appDiscordRole = DiscordRoleMapper.Map(role);
    ...
    {
        ctx.Guild.Roles.TryGetValue(roleId, out DiscordRole? discordRole);
    ...
    {
        TryResolveResult<DiscordRole> result = _discordResolver.TryResolveRoleByName(ctx.Guild, discordRoleName);
    ...
    {
        IEnumerable<AppDiscordRole> guildRoles = ctx.Guild.Roles.Select(r => DiscordRoleMapper.Map(r.Value));

        UserRoleService.SyncRolesResult result = await _userRoleService.SyncRoles(guildRoles);
