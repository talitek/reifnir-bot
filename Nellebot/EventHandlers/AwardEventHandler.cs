...
public class AwardEventHandler
{
    ...
    private Task OnMessageReactionAdded(DiscordClient client, MessageReactionAddEventArgs eventArgs)
    {
        ...
        try
        {
            if (!ShouldHandleReaction(channel, user)) return Task.CompletedTask;

            var isAwardEmoji = emoji.Name == EmojiMap.Cookie;

            if (!IsAwardAllowedChannel(channel)) return Task.CompletedTask;

            return !isAwardEmoji
                ? Task.CompletedTask
                : _awardQueue.Writer.WriteAsync(new MessageAwardItem(message, MessageAwardQueueAction.ReactionChanged))
                    .AsTask();
        }
        catch (Exception ex)
        {
    ...
    private Task OnMessageReactionRemoved(DiscordClient sender, MessageReactionRemoveEventArgs eventArgs)
    {
        ...
        try
        {
            if (channel.IsPrivate) return Task.CompletedTask;
    ...
    private Task OnMessageUpdated(DiscordClient sender, MessageUpdateEventArgs eventArgs)
    {
        try
        {
            ...
            return !IsAwardAllowedChannel(channel)
                ? Task.CompletedTask
                : _awardQueue.Writer.WriteAsync(new MessageAwardItem(message, MessageAwardQueueAction.MessageUpdated))
                    .AsTask();
        }
        catch (Exception ex)
        {
    ...
    private Task OnMessageDeleted(DiscordClient sender, MessageDeleteEventArgs eventArgs)
    {
        try
        {
            ...
            {
                return _awardQueue.Writer
                    .WriteAsync(new MessageAwardItem(messageId, channel, MessageAwardQueueAction.MessageDeleted))
                    .AsTask();
            }
            ...
            {
                return _awardQueue.Writer
                    .WriteAsync(new MessageAwardItem(messageId, channel, MessageAwardQueueAction.AwardDeleted))
                    .AsTask();
            }
